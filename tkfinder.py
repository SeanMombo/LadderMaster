...
def get_character(chara_name: str) -> dict:
    ...
    filepath = dirname + "/t7_frame_data/character_misc.json"
...
def get_move(character: dict, move_command: str, case_important: bool) -> dict:
    ...
    move_file_name = "/t7_frame_data/" + character.get("local_json")
    ...
    if case_important:
        ...
        move = list(
            filter(
                lambda x: (
                    move_simplifier(x["Command"].lower())
                    == move_simplifier(move_command.lower())
                ),
                move_json,
            )
        )
        if not move:
            move = list(
                filter(
                    lambda x: move_simplifier(move_command.lower())
                    in move_simplifier(x["Command"].lower()),
                    move_json,
                )
            )
    if move:
